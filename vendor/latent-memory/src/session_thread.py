...
跟 recall_recent 的分工：
  ...
  thread         一个单元，"上次那条线聊到哪、当下什么状态、有什么没聊完"——精度
换窗时两层一起给：thread 在前（精度），召回块在后（广度）；没有 thread 就只有
召回块，不断线。接线在 session_recall.SessionRecall。
...
class Thread:
    """一条会话线索。字段各防一种失效：
      ...
      open_loops    有什么没聊完，换窗后最该先捡起来的东西
...
def format_thread_block(thread: Thread, now=None, time_context=None):
    ...
    lines.append("- 当下状态：" + thread.current_state)
    if thread.open_loops:
        lines.append("- 没聊完：" + "；".join(thread.open_loops))
...
def _selftest():
    ...
    assert "花买好了" in block and "周末去哪还没定" in block
